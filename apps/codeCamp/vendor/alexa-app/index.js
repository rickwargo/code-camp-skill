...
alexa.request = function (json) {
    ...
    this.slot = function (slot_name, default_value) {
        try {
            return this.data.request.intent.slots[slot_name].value || default_value;
        } catch (e) {
...
alexa.app = function (name, endpoint) {
    ...
    this.messages = {
        ...
        // When the application id is not right
        , "BAD_APP_ID": "Error: not a valid request"
            ...
            try {
                ...
                if (typeof self.pre == "function") {
                    if (response.sessionAttributes) {
                        request.sessionAttributes['$Context'] = [];  // RCW: Initialize the context
                    }
                ...
                if (!response.resolved) {
                    if ("IntentRequest" === requestType) {
                        ...
                        if (typeof self.intents[intent] != "undefined" && typeof self.intents[intent]['function'] == "function") {
                            var path = response.session('$Context');
                            if (path.length > 1) {
                                response.session('$Referrer', path[path.length - 2]);  // RCW: Make the previous Intent easily accessible
                                                                                                 // and do not override if existing (usually from tests)
                            }
                            response.session('$Intent', intent);  // RCW: Make the current Intent easily accessible
                            response.session('$Context').push(intent); // RCW: Keep a chain of the followed intents
                            //if (self.persistentSession) {   // RCW: Copy newly added session attribute if requested to persist
                            //    response.session('$Context', request.sessionAttributes['$Context']);
                            //    response.session('$Intent', request.sessionAttributes['$Intent']);
                            //    response.session('$Referrer', request.sessionAttributes['$Referrer']);
                            //}
